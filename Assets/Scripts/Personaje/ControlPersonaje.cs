...
public class ControlPersonaje : MonoBehaviour {
   ...
   private bool DobleSalto = true;  //variable booleana para verificar si dispone del doble salto
    ...
    //Si el personaje se mantiene sobre esa colision (suelo)
    void OnCollisionEnter2D(Collision2D collision)   
    {
        ...
        if (collision.gameObject.tag == "Limite"  || collision.gameObject.tag == "Meteorito" 
            || collision.gameObject.tag == "Agua" || collision.gameObject.tag == "Orco") 
        {
            //si el personaje se cae al vacío, colisiona con el agua o un meteorito pierde y pierde el puntaje
    ...
    //si el personaje deja de colisionar con suelo entonces suelo = false
    ...
    void Update () {
        ...
        if (mover>0)    //si mover es 1 (se mueve a la derecha) entonces no se da vuelta el sprite 
        {
    ...
    private void OnTriggerEnter2D(Collider2D collision) //si colisiona con un objeto
    {
        ...
        //si el puntaje supera los 50 o los 100 suma una vida
        if(Puntaje.score == 50 || Puntaje.score == 100){
            vidas.sumar();
        }
